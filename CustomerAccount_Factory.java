...
public class CustomerAccount_Factory extends WebPageFactory {

    List<List<Integer>> accountsForAllUsers = new ArrayList<>(5);

    public CustomerAccount_Factory(WebDriver driver) {
        super(
                driver,
...
// DOERS
    ...
    public void depositMoney(WebDriver driver, int amount) {
        ...
        new WebDriverWait(driver, 10).until(ExpectedConditions.elementToBeClickable(this.getDepositComponentButton()));
    ...
    public void withdrawMoney(WebDriver driver, int amount) {
        if (!driver.getCurrentUrl().equals(this.getUrl())) {
            driver.navigate().to(this.getUrl());
        }
        int amount_beforeWithdraw = Integer.parseInt(this.getAccountInfo().get(1).getText());
        this.getButton_Withdraw().click();
        new WebDriverWait(driver, 10).until(ExpectedConditions.elementToBeClickable(this.getWithdrawComponentButton()));
